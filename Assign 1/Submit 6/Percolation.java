...
public class Percolation {
    ...
    private void fillSites (int i, int j) {
        ...
        if (y == 1){
            wquf.union(0, n);
        }else if (i == 1 && isOpen(i, j) && !isFull(i, j)) {
            wquf.union(convert2dTo1d(i, j), 0);
            if (i + 1 <= y && isOpen(i + 1, j) && !isFull(i + 1, j)) fillSites(i + 1, j);

        }else if (i == y ){
            if (i - 1 > 0 && isFull(i - 1, j)) {
                wquf.union(convert2dTo1d(i, j), n);
                wquf.union(convert2dTo1d(i, j), convert2dTo1d(i - 1, j));
                if (j + 1 <=y && isOpen(i, j + 1) && !isFull(i, j + 1)) fillSites (i, j + 1);
                if (j - 1 >= 0 && isOpen (i, j - 1) && !isFull(i, j - 1)) fillSites (i, j - 1);
            ...
            if ((i - 1) > 0 && isFull(i - 1, j)) {
                ...
                wquf.union(convert2dTo1d(i, j), convert2dTo1d(i, j - 1));
                if (i - 1 > 0 && isOpen(i - 1, j) && !isFull(i - 1, j)) fillSites(i - 1, j);
                if (j + 1 <= y && isOpen(i, j + 1) && !isFull(i, j + 1)) fillSites(i, j + 1);
                if (i + 1 <= y && isOpen(i + 1, j) && !isFull(i + 1, j)) fillSites(i + 1, j);
            }else if ((j + 1) <= y && isFull(i, j + 1)) {
                wquf.union(convert2dTo1d(i, j), convert2dTo1d(i, j + 1));
